...
conn_path='/Volumes/HD1/Positivity_Project/Positivity_fMRI_Analysis/Exploration_fMRI_Analysis/Single_Beta_Analysis/WM_CSF_confounds'
beta_path='/Volumes/HD1/Positivity_Project/Positivity_fMRI_Analysis/Exploration_fMRI_Analysis/Single_Beta_Analysis/mean'
...
# hans macbook path
fmri_path=`echo "/Volumes/HD1/Positivity_Project/Positivity_fMRI_DATA/MOOD1MR/"*_$SubNo`
...
fast -t 1 -n 3 -H 0.1 -I 4 -l 20.0 -g --nopve -o $conn_path/$SubNo/${SubNo} $struct_file
...
flirt -in $conn_path/$SubNo/${SubNo}_seg_2.nii.gz -ref /usr/local/fsl/data/standard/MNI152_T1_2mm_brain -out $conn_path/$SubNo/${SubNo}_standard_WM_mask -omat $conn_path/$SubNo/WM_CSFconfounds/${SubNo}_standard_WM_mask.mat -bins 256 -cost corratio -searchrx -180 180 -searchry -180 180 -searchrz -180 180 -dof 12  -interp trilinear

echo 'Transform CSF map for' $SubNo
flirt -in $conn_path/$SubNo/${SubNo}_seg_0.nii.gz -ref /usr/local/fsl/data/standard/MNI152_T1_2mm_brain -out $conn_path/$SubNo/${SubNo}_standard_CSF_mask -omat $conn_path/$SubNo/WM_CSFconfounds/${SubNo}_standard_CSF_mask.mat -bins 256 -cost corratio -searchrx -180 180 -searchry -180 180 -searchrz -180 180 -dof 12  -interp trilinear

## Create binary masks out of standard space WM and CSF maps

echo 'Binarize WM map to create mask for' $SubNo
fslmaths $conn_path/$SubNo/${SubNo}_standard_WM_mask -thr 0.9 -bin $conn_path/$SubNo/${SubNo}_standard_WM_mask90_bin

echo 'Binarize CSF map to create mask for' $SubNo
fslmaths $conn_path/$SubNo/${SubNo}_standard_CSF_mask -thr 0.9 -bin $conn_path/$SubNo/${SubNo}_standard_CSF_mask90_bin
...
fslmeants -i $beta_path/mcdcdn_${SubNo} -m $conn_path/$SubNo/${SubNo}_standard_WM_mask90_bin -o $conn_path/$SubNo/${SubNo}_WM_confound_meants.txt

fslmeants -i $beta_path/mcdcdn_${SubNo} -m $conn_path/$SubNo/${SubNo}_standard_CSF_mask90_bin -o $conn_path/$SubNo/${SubNo}_CSF_confound_meants.txt
