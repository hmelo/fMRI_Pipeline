...
SubNo=cell2mat(C{1}(i)); SubNo=SubNo(20:22); % Get SubNo
[choice_rwd, choice,b,trial,rt] = import_data_EEG(file_name); % import relevant data
...
data=NaN(75,4);
data(:,1)=repmat(str2double(SubNo), 75,1);
data(:,2)=trial;
...
data(:,7)=repmat(exploration, 75,1); % exploration parameter for each subject
...
dlmwrite('all_subs_EEG_data.txt',headers,'-append','delimiter','')
dlmwrite('all_subs_EEG_data.txt',all_subs_data,'-append','delimiter','\t')
