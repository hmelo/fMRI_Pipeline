...
regs=[exploit exploit==0]';

subj = set_mat(subj,'regressors','conds',regs);
condnames = {'exploit','explore'};
...
% % % subj = set_objfield(subj,'regressors','conds','condnames',condnames_tp1);
% % % 
% % % % for Choice_Cat
% % % 
% % % subj = init_object(subj,'regressors','conds_ChoiceCat');
% % % 
% % % % conditions loaded directly from csv file
% % % % load('tutorial_regs');
% % % choice1=ChoiceCat==1;
...
% % % 
% % % subj = set_mat(subj,'regressors','conds_ChoiceCat',regs_ChoiceCat);
% % % condnames_ChoiceCat = {'choice1','choice2','choice3','choice4','choice5'};
% % % subj = set_objfield(subj,'regressors','conds','condnames',condnames_ChoiceCat);
% % % 
...
[subj] = feature_select(subj,'fmri_z','conds','trials_xval');
...
[subj results] = cross_validation(subj,'fmri_z','conds','trials_xval','fmri_z_thresh0.05',class_args);
...
view_montage(subj,'pattern','anat','mask','DLPFC_category-selective')

view_montage(subj,'pattern','anat','mask','fmri_z_thresh0.05_2')

view_montage(subj,'mask','wholebrain2','mask','DLPFC_category-selective')

view_montage(subj,'mask','wholebrain2','mask','fmri_z_thresh0.05_2');
