...
path="/Volumes/HD1/Positivity_Project/Positivity_fMRI_Analysis/Exploration_fMRI_Analysis/FIR/onsets/${SubNo}"
mkdir $path
path2="/Volumes/HD1/Positivity_Project/Positivity_fMRI_DATA/fMRI Behav/Dec. 3. 2013/fMRI Tasks/Exploration/results"
path3="/Volumes/HD1/Positivity_Project/Positivity_fMRI_Analysis/Exploration_fMRI_Analysis/modelling/indirectActor/modelled_data"
...
for c in $exploit; do
	if [ "$c" = "1" ]; then
  	  	echo "${onset[in]} 3.0 $c" >> exploit_only.txt;
    elif [ "$c" = "0" ]; then
	 	echo "${onset[in]} 3.0 1" >> explore_only.txt;
 	else
	 echo "${onset[in]} 3.0 1" >> no_response_only.txt;
 fi
  ((in++))
done
